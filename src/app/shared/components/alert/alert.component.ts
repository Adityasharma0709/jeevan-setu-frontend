...
@Component({
  ...
  standalone: true,
  imports: [ZardIconComponent, ZardStringTemplateOutletDirective, CommonModule],
  template: `
    @if (zIcon() || iconName()) {
      <span [class]="iconClasses()" data-slot="alert-icon">
        <ng-container *zStringTemplateOutlet="zIcon()">
          <z-icon [zType]="iconName()!" class="size-4!" />
        </ng-container>
      </span>
    }
...
export class ZardAlertComponent {
  ...
  protected readonly iconName = computed((): ZardIcon | undefined => {
    const customIcon = this.zIcon();
    if (customIcon && !(customIcon instanceof TemplateRef)) {
      return customIcon as ZardIcon;
    }
