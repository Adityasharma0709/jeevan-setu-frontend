...
import { ManagerService, ManagerBeneficiary } from '../manager.service';
...
export class Beneficiaries implements OnInit, OnDestroy {
    ...
    map(([beneficiaries, page, sortCol, sortDir, search]) => {
      ...
      if (search) {
        items = items.filter((b) =>
          ...
          (b.createdBy?.name && b.createdBy.name.toLowerCase().includes(search))
        );
      }
  ...
  editRecord(beneficiary: ManagerBeneficiary): void {
    if (this.isManager) return;
  ...
  trackById(_: number, item: ManagerBeneficiary): number {
    return item.id;
  }
  //===================Quick Access===========================
  ...
  getBeneficiaryType(b: ManagerBeneficiary): string {
  ...
  private filterBeneficiariesByDateRange(beneficiaries: ManagerBeneficiary[]): ManagerBeneficiary[] {
  ...
  async exportToExcel(): Promise<void> {
    ...
    let exportBeneficiaries: ManagerBeneficiary[] = [];
